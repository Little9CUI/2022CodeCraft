...
public:
	...
	vector<unordered_map<string, unordered_set<string>>> streamSource;//记录每一个时刻，每一个流的来源是哪些服务器
...
//outSerName在time时刻想要降低到target，目标是全降低，最后返回的时候如果能降到target就够了
	...
	//判断一个流是否能装进去，动态调整用
	bool IfCanPutAdjust(string serName, int time, int streamNum, int constrain);
	//得到服务器新的上限
